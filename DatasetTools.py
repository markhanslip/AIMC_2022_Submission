...
class AudioDataProcessing:
    ...
    def __init__(self, name: str, in_file: str, train_dir:str): # change to accept a category name only
        self.classname = name
        ...
        for i, n in enumerate(data):
            if i % chunk_len == 0:
                if len(data) - startpos >= 32768:
                    count+=1
                    sf.write(os.path.join(self.chunk_dir,'{}.wav'.format(str(count).zfill(6))), samplerate=sr, data=data[startpos:endpos])
